...
import homeworks.homework_4.full_sequence.fullSequence
...
/**
 * Каждый метод принимающий implicit параметр ExecutionContext
 * внутри будет выполнен как отдельный Runnable в переданном execution context
 * в данном задании scala.concurrent.ExecutionContext.Implicits.global
 * инструментирован счетчиком таких Runnable
 * Чем меньше Runnable создастся, тем меньше работы будет на контексте выполнения
 * Ваша задача постараться предоставить максимально эффективную реализиацию по возможности,
 * вы можете проверить себя задав соответствующий limit на количество создаваемых внутри Runnable
 * */

class full_sequence_test extends AnyFlatSpec {
